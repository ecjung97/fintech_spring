...
public class BoardController {//스프링 MVC 게시판 컨트롤러 클래스
	...
	public ModelAndView board_cont(@RequestParam("bno") int bno, int page) {
		/* @RequestParam("bno")는 request.getParameter("bno") 와 기능이 같다. 즉 bno 피라미터 이름에 저장되어 져서 전달된 번호값을
		 * 가져온다. int page도 page피라미터 이름에 저정되어 져서 전달된 쪽번호를 가져온다.
		 */
		
		BoardVO bc = this.boardService.getBoardCont(bno);//내용보기와 조회수 증가
		String bcont = bc.getContent().replace("\n","<br/>");
		//textarea에서 엔터키를 친 부분을 줄바꿈해준다.
		
		ModelAndView cm = new ModelAndView();
		cm.addObject("bc", bc);
		cm.addObject("page", page);//페이징 목록에서 책갈피 기능때문에 쪽번호 저장
	...
	public void board_edit(Model m, int bno, int page) {
		//int bno라고 하면 bno피라미터 이름에 저장되어 get으로 전달된 게시판 번호값을 받아온다.
	...
	public String board_edit(BoardVO eb, int page, RedirectAttributes rttr) {
		/* BoardVO eb라고 하면 빈클래스 변수명과 네임피라미터 이름이 같으면 eb객체에 히든으로 전달된 번호, 수정글쓴이,수정제목,수정내용이 
		 * 저장되어 있다. 하지만 히든 page는 BoardVO 빈클래스에 변수로 정의 되어 있지 않아서 별도로 int page로 가져와야 한다.
		 */
		this.boardService.updateBoard(eb);//번호를 기준으로 수정
