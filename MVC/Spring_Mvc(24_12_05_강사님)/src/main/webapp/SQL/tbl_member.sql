...
--사이트 완성후 한 고객이 게시판 내용 밑의 댓글수를 카운터해서 출력해 달라는 요구사항=>tbl_board 게시판테이블에 댓글수를 카운터해서 저장할 컬럼
--replycnt를 추가
alter table tbl_board
add (replycnt int default 0);

--tbl_reply 댓글테이블의 게시판번호에 해당하는 댓글수를 카운터해서 tbl_board테이블에 생성된 replycnt컬럼 레코드로 업데이트
...
create table tbl_reply(
 ...
 , bno number(38) default 0 --외래키(foreign key)제약조건으로 추가설정. tbl_board 게시판 테이블의 bno컬럼 번호값만 저장됨.
...
create table tbl_message(
  ...
  , sender varchar2(30) not null --메시지를 보낸 사람
