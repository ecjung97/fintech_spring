...
function id_check(){
	...
	*/
	...
	$.ajax({//비동기식 아작스 실행
		...
		data: {"id":$mem_id}, //id 피라미터 이름에 회원아이디를 저장
		datatype:'int',//서버로 부터 받아오는 타입
		success:function(result){//받아오는 것이 성공시 호출되는 콜백함수이다. 받아온 데이터는 매겨변수 result에 저장
			//alert(result);
		...
		error:function(){//비동기식 아작스로 백엔드 서버 데이터를 못가져와서 에러가 발생했을 때 호출되는 콜백함수이다.
			alert('data error');
		}
	});//$.ajax({})
}//idcheck();
